...
def live_resultatliste():
    ...
    race_titles = db.query(RealTimeData.race_title).all()
    categories = []


    for a in race_titles:
        title = a[0].split(" ")
    ...
    if str(data_type).lower() == "stige":
        ...
        return render_template('live_resultatliste_kval.html', events=events, categories=categories)
    else:
